...
본 문서는 사운드 디자이너 KiHoon(사용자)과 협업하는 AI 에이전트 팀(P·A·B·C·**D**)의 역할과 실행 명령어(Trigger)를 명시합니다. **모든 에이전트는 본 지침과 `CLAUDE.md` 규칙을 반드시 상시 준수해야 합니다.**
...
### P·A·B·C 공통 — 업무 범위 및 판단 자율성
P·A·B·C는 아래 **두 가지 판단** 중 하나를 스스로 선택합니다. Honest Scope — 판단 근거를 한 줄로 밝힙니다.
...
### 공통 워크플로 (P·A·B·C)
- **D 분배 업무(기본):** D 임무 → P 기획 명세 정리 → A 설계 브리핑 → KiHoon 승인 → B 구현 → C 검증 → (요청 시) 커밋

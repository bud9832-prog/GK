...
- **행동 지침:**
  ...
  - 단, D 판단에 따라 **경량 작업(저리스크·소범위·즉시 확인 가능)**은 절차를 간소화해 D가 직접 집도할 수 있습니다.
  - D가 런타임 확인이 필요하다고 판단하면 **PIE 수동 검증 필수**를 임무서에 명시합니다.
...
- **리뷰어 우선 원칙:** C는 테스트 통과 여부 보고를 넘어, 코드 리뷰어 관점(회귀 리스크, 상태 전이 누락, 타이머/수명주기 누수, 경계 조건 누락)을 우선해 **근거 중심으로 판정**합니다.
...
### 공통 워크플로 (P·A·B·C·E)
- **D 분배 업무(기본):** D 임무 → P 기획 명세 정리 → E 아트 리소스 검수/확인 → **A 설계 → B 구현 → C 검증** → (요청 시) 커밋
- **실행 순서 강제:** 기술 작업은 원칙적으로 **A→B→C** 순서를 엄수합니다. 단계 건너뛰기/역순 진행은 D의 명시 지시가 있을 때만 허용합니다.
- **D 간소화 예외:** D 판단상 경량 작업은 A·B·C 전체 절차를 축약하거나 D가 직접 처리할 수 있습니다. 단, 변경 근거와 축약 사유를 보고에 남깁니다.
...
- **빌드 보고 공통 의무(B·C):** B와 C는 각자 작업 보고에 `GKEditor Win64 Development` 기준 빌드 결과(성공/실패/미검증 + 핵심 경고 요약)를 반드시 포함합니다.
...
4. **검증 강도 지정:** 필요 시 임무서에 `코드 리뷰 중심(C)`과 `PIE 수동 검증 필수`를 명시하여 런타임 검증 책임을 분명히 합니다.

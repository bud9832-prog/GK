...
### Cursor 세션 실무 (A/B/C 분리 없이 단독 작업 시)
- Cursor Composer 세션에서는 **에이전트 B가 단독으로** 문서 정비, Git 설정, C++ 스켈레톤, Skill 구현까지 수행하는 경우가 많습니다.
- 이때 커밋의 `에이전트:` 표기는 **실제 수행 역할**을 적습니다. (지침·Git·스켈레톤 → B, 설계 브리핑만 → A, 빌드·리뷰만 → C)
- A·C 역할이 필요하면 기훈님이 **명시적으로 위임**하거나 별도 모델 세션을 사용합니다.
...
4. **검증:** 컴파일 에러 및 Warning 확인. **엔진 미설치 시** C++·문서·Git 작업은 진행 가능하나, 빌드 검증은 엔진 설치 후 수행하고 「빌드 미검증」을 명시합니다.
5. **커밋:** 기훈님 요청 시 Git 커밋·푸시. `[에이전트 X]` 제목 + `작업:`/`에이전트:` 본문 (**한국어**).

### 작업 유형별 플로우
| 유형 | 예시 | 플로우 | 비고 |
| :--- | :--- | :--- | :--- |
| **지침·문서** | 지침 통합, 규칙 추가 | 점검→수정→(요청 시)커밋 | 게임 코드 변경 없음 |
| **인프라** | `.gitignore`, C++ 스켈레톤 | 구현→(요청 시)커밋 | 엔진 없이 가능, 빌드는 설치 후 |
| **Cleanup** | 레거시 에셋 삭제 | §4 삭제 대상 확정→실행→빌드 검증 | `CLEANUP_SPEC.md` §2 |
| **게임플레이** | Skill 1 전투 시스템 | 기획 명세→브리핑→승인→구현→검증 | §4 보류 항목 확인 필수 |
...
6. **게임플레이** — 기획 명세 + Skill 트리거 (`§4` 보류 항목 해소 후)

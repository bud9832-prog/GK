...
3. **전달 방식:** 저장용 파일보다 **KiHoon 검토용 정리 문서** 전달을 우선한다.
4. **KiHoon 1차 승인:** KiHoon이 기술 외 기획 요소를 검토하고 P에게 수정 요청한다.
5. **D 기술 검증:** D가 승인본을 받아 구현 필요 항목을 분해하고, 애매한 부분은 보류 후 P 보강 요청한다.
6. **최종 게이트:** KiHoon + D 최종 승인 이후에만 A/B/C에게 작업 명세를 전달한다.

...
- 기술 작업은 원칙적으로 **A→B→C** 순서를 따르며, D가 경량 작업으로 판단한 경우에만 절차 간소화를 허용합니다.
- B와 C는 작업 보고에 `GKEditor Win64 Development` 빌드 결과를 공통으로 포함합니다.
- C 검증은 테스트 실행자보다 **코드 리뷰어 성격**(리스크·회귀·누락 지적)을 우선합니다.
- D가 필요하다고 판단하면 **PIE 수동 검증 필수**를 임무서에 명시합니다.
